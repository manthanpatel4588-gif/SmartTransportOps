...
**SmartTransportOps** is a premium, real-time fleet dispatch control and operational expense tracking enterprise single-page web application. It is designed to optimize logistical transport management by organizing drivers, tracking vehicle inventory, orchestrating transit routes, and analyzing operational expenditures—all inside a highly responsive, modern B2B Enterprise SaaS light-themed ERP console inspired by Stripe, Ramp, and Linear.
...
* **Safety Tracker**: Circular SVG progress gauges highlight operator safety scores.
* **Alert Alerts**: Automatic visual flags highlight expired or soon-to-expire licenses.
* **Layout Switcher**: Toggle between grid list and profile cards with one click.

### 🚚 Fleet Inventory
* **Inventory Panel**: Track vehicle model specs, license plates, and capacities.
* **Status Badges**: Monitors whether trucks are `On Duty`, `Idle`, or in `Maintenance`.
* **Sliding Drawer Registration**: A smooth drawer sliding out from the right to add new vehicles.
...
* **Frontend Framework**: [React](https://react.dev/) (Hooks: `useState`, `useMemo`, `useEffect`, `useRef`)
* **Build System**: [Vite](https://vite.dev/) (optimized ESBuild compiling)
* **Styling System**: [Tailwind CSS v4](https://tailwindcss.com/) & Vanilla CSS (Solid color palettes, clean borders, 20px card radius, collapsible navigation drawers, Recharts, SVG visual score rings, custom keyframe micro-animations)
